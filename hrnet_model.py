...
# ==============================================================================
"""Builds a HRNet model for image classification."""
...
def high_resolution_module(inputs, num_branches, num_inchannels,
...
class Model(object):
  ...
  def __init__(self, hrnet_size, bottleneck, num_classes, num_filters,
               kernel_size, conv_stride, module_sizes, block_sizes,
               base_channel, final_conv_channel, data_format=None,
               dtype=DEFAULT_DTYPE):
    ...
    Args:
      ...
      block_sizes: A list containing n values, where n is the number of sets of
        ...
        i-th set.
      base_channel: The base_channel in classification head.
      final_conv_channel: The channel number of the final conv layer.
  ...
  def __call__(self, inputs, training):
    ...
    with self._model_variable_scope():
      ...
      for i, num_modules in enumerate(self.module_sizes[1:], 2):
        with tf.variable_scope('stage{}'.format(i)):
          ...
          for j in range(num_modules):
            stagex_input = high_resolution_module(
      ...
      with tf.variable_scope('cls_head'):
        for i, input in enumerate(stagex_input):
          with tf.variable_scope('branch{}'.format(i)):
            output = block_layer(
              stagex_input[i], inp_filters=num_inchannels[i],
              filters=num_inchannels[i], bottleneck=True,
              block_fn=_bottleneck_block_v1, blocks=1, strides=1,
              training=training, name='head_branch_{}'.format(i),
              data_format=self.data_format)
          outputs.append(output)

        for i in range(num_branches - 1):
          dw_output = conv2d_fixed_padding(
            inputs=outputs[i], filters=num_inchannels[i + 1] * 4,
            kernel_size=3, strides=2, data_format=self.data_format)
          dw_output = batch_norm(dw_output, training, self.data_format)
          dw_output = tf.nn.relu(dw_output)

          outputs[i + 1] += dw_output

        final_output = conv2d_fixed_padding(
          outputs[-1], filters=self.final_conv_channel,
          kernel_size=3, strides=1, data_format=self.data_format)
        ...
        inputs = tf.reduce_mean(input_tensor=final_output, axis=axes, keepdims=True)
